/*
  ...
  2. Security
    ...
    - Only the service role can insert audits (done via edge function)
    - Leads are write-only from anon (insert only), service role reads all
*/
...
CREATE POLICY "Authenticated and anon can insert audits"
...
CREATE POLICY "Anon and authenticated can insert leads"
  ON leads FOR INSERT
  TO anon, authenticated
  WITH CHECK (honeypot = '');
